...
BEGIN {
    ...
    if ($] < 5.006) {
        ...
        if ($^O ne 'MacOS') {
            *CORE::GLOBAL::binmode =
            *CORE::GLOBAL::binmode = \&Elatin10::binmode;
        }
        *CORE::GLOBAL::open    =
        *CORE::GLOBAL::open    = \&Elatin10::open;
    }
}
...
if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
    if ($ENV{'ComSpec'} =~ / (?: COMMAND\.COM | CMD\.EXE ) \z /oxmsi) {
        ...
        for (@ARGV) {
            ...
            elsif (m/\A (?:$q_char)*? [*?] /oxms and (my @glob = Elatin10::glob($_))) {
                push @argv, @glob;
            }
            else {
                push @argv, $_;
...
sub _octets {
    ...
    if ($length == 1) {
        ...
        if (((caller(1))[3] ne 'Elatin10::_octets') and ($modifier =~ m/i/oxms)) {
            if ($a == $z) {
...
#               $pattern .= "(?:$your_char)",   # UNIX style
            ...
            elsif ((my $uc = Elatin10::uc($char)) ne $char) {
                $pattern .= $uc;
            }
            else {
        ...
        my $matchsub = sub { Elatin10::uc($_[0]) =~ m{\A $pattern \z}xms };
...
This module is a run-time routines of the Latin10.pm.
Because the Latin10.pm automatically uses this module, you need not use directly.
...
=item Split string
  ...
  $string on every match. If the pattern doesn't match at all, Elatin10::split returns
...
=item Chop string
  ...
  The Elatin10::chop operator is used primarily to remove the newline from the end of
  an input record but is more efficient than s/\n$//. If no argument is given, the
  function chops the $_ variable.

=item Index string

  $pos = Elatin10::index($string,$substr,$position);
  $pos = Elatin10::index($string,$substr);
  ...
  the Elatin10::index function returns -1.

=item Reverse index string

  $pos = Elatin10::rindex($string,$substr,$position);
  $pos = Elatin10::rindex($string,$substr);

  Works just like Elatin10::index except that it returns the position of the last
...
=item Make character
  ...
  set. For example, Elatin10::chr(65) is "A" in either ASCII or Latin-10, and
  Elatin10::chr(0x82a0) is a Latin-10 HIRAGANA LETTER A. For the reverse of Elatin10::chr,
  use Latin10::ord.
...
=item binary mode (Perl5.6 emulation on perl5.005)
  ...
  Elatin10::binmode(FILEHANDLE, $disciplines);
  Elatin10::binmode(FILEHANDLE);
  Elatin10::binmode($filehandle, $disciplines);
  Elatin10::binmode($filehandle);
...
=item open file (Perl5.6 emulation on perl5.005)
  ...
  $rc = Elatin10::open(FILEHANDLE, $mode, $expr);
  $rc = Elatin10::open(FILEHANDLE, $expr);
  $rc = Elatin10::open(FILEHANDLE);
  $rc = Elatin10::open(my $filehandle, $mode, $expr);
  $rc = Elatin10::open(my $filehandle, $expr);
  $rc = Elatin10::open(my $filehandle);
  ...
  * three arguments
  ...
  {
      my $fh;                          # (uninitialized)
      Elatin10::open($fh, ">logfile")     # $fh is autovivified
  ...
  Elatin10::open my $fh, ">logfile" or die ...
  ...
  Elatin10::open(LOG, ">", "logfile") or die "Can't create logfile: $!";
  ...
  $path =~ s#^([ ])#./$1#;
  Elatin10::open (FH, "< $path\0") or die "can't open $path: $!";
  ...
  Elatin10::open(FH, "<", $path) or die "can't open $path: $!";
  ...
  Elatin10::open(FH, "<:raw", $path) or die "can't open $path: $!";
  ...
  Elatin10::open(FH, "<:crlf:encoding(Latin10)", $path) or die "can't open $path: $!";
