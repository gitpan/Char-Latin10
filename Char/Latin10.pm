...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ m/\A [\x00-\xFF] \z/oxms) and (Char::Elatin10::uc($char[$i]) ne Char::Elatin10::lc($char[$i]))) {
            $char[$i] = '[' . Char::Elatin10::uc($char[$i]) . Char::Elatin10::lc($char[$i]) . ']';
        }
...
http://search.cpan.org/dist/Char-Char::Latin10/Char::Latin10/JA.pm
...
Definitions in Char/Elatin10.pm.
  ...
  @{Char::Elatin10::eV}             qr{(?:[^\x0C\x0A\x0D])}
