...
#
# http://search.cpan.org/dist/Char-Char::Latin10/
#
...
BEGIN { CORE::require Char::Elatin10; }
...
sub unimport {}
sub Char::Latin10::escape_script;
...
my $function_ord;         # ord()   to ord()   or Char::Latin10::ord()
my $function_ord_;        # ord     to ord     or Char::Latin10::ord_
my $function_reverse;     # reverse to reverse or Char::Latin10::reverse
my $function_getc;        # getc    to getc    or Char::Latin10::getc
...
END
    ...
    print Char::Latin10::escape_script($ARGV[0]);
    exit 0;
}
...
sub import {
    ...
    if (not -e("$filename.e")) {
        ...
        if (eval q{ use Fcntl qw(O_WRONLY O_APPEND O_CREAT); 1 } and CORE::sysopen($fh,"$filename.e",&O_WRONLY|&O_APPEND|&O_CREAT)) {
            ...
            Char::Elatin10::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e";
        }
        ...
        my $e_script = Char::Latin10::escape_script($filename);
        print {$fh} $e_script;
    ...
    my $fh = gensym();
    Char::Elatin10::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e";
...
sub Char::Latin10::escape_script {
    ...
    my $fh = gensym();
    Char::Elatin10::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die __FILE__, ": Can't close file: $script";

    if (/^ use Char::Elatin10(?:\s+[0-9\.]*)?\s*; $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Char::Elatin10 %s;\n", $Char::Latin10::VERSION); # require run-time routines version

        # use Char::Latin10 version qw(ord reverse getc);
        $function_ord     = 'ord';
        $function_ord_    = 'ord';
        ...
        if (s/^ \s* use \s+ Char::Latin10 \s* ([^;]*) ; \s* \n? $//oxms) {
            ...
            if ($list =~ s/\A ([0-9]+\.[0-9]+) \.0 \s* //oxms) {
                my $version = $1;
                if ($version ne $Char::Latin10::VERSION) {
                    my @file = grep -e, map {qq{$_/Char/Latin10.pm}} @INC;
...
END
                    ...
                    die "Script $0 expects Char/Latin10.pm $version, but @{[__FILE__]} is version $Char::Latin10::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Char/Elatin10.pm $version, but \\\$Char::Elatin10::VERSION is \$Char::Elatin10::VERSION" if \$Char::Elatin10::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ([0-9]+(?:\.[0-9]*)) \s* //oxms) {
                my $version = $1;
                if ($version > $Char::Latin10::VERSION) {
                    die "Script $0 required Char/Latin10.pm $version, but @{[__FILE__]} is only version $Char::Latin10::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'Char::Latin10::ord'     if /\A ord \z/oxms;
                    $function_ord_    = 'Char::Latin10::ord_'    if /\A ord \z/oxms;
                    $function_reverse = 'Char::Latin10::reverse' if /\A reverse \z/oxms;
                    $function_getc    = 'Char::Latin10::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }
...
# escape Latin-10 part of script
sub escape {
...
# functions of package Char::Elatin10
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                        2   3                                 4   5
                q<eval{local $Char::Latin10::re_t=%s; while($Char::Latin10::re_t =~ %s){%s local $^W=0; local $Char::Latin10::re_r=%s; %s$Char::Latin10::re_t="$`$Char::Latin10::re_r$'"; pos($Char::Latin10::re_t)=length "$`$Char::Latin10::re_r"; } return $Char::Latin10::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Char::Latin10::re_r=eval $Char::Latin10::re_r; ' x $e_modifier,                  #  5
            );
        }
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2          3                                 4   5  6                    7
                q<(%s =~ %s) ? eval{%s local $^W=0; local $Char::Latin10::re_r=%s; %s"%s$Char::Latin10::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                 1     2   3                                 4   5 6                         7                                                 8
                q<eval{local $Char::Latin10::re_n=0; while(%s =~ %s){%s local $^W=0; local $Char::Latin10::re_r=%s; %s%s="$`$Char::Latin10::re_r$'"; pos(%s)=length "$`$Char::Latin10::re_r"; $Char::Latin10::re_n++} return %s$Char::Latin10::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3          4                                 5   6 7   8
                q<(%s%s%s) ? eval{%s local $^W=0; local $Char::Latin10::re_r=%s; %s%s="%s$Char::Latin10::re_r$'"; 1 } : undef> :

                #  1 2 3              4                                 5   6 7   8
                q<(%s%s%s) ? 1 : eval{%s local $^W=0; local $Char::Latin10::re_r=%s; %s%s="%s$Char::Latin10::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Char::Latin10::re_r=eval $Char::Latin10::re_r; ' x $e_modifier,                  #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
=item * You'll have to download Char/Latin10.pm and Char/Elatin10.pm and put it in your perl lib directory.

=item * You'll need to write "use Char::Latin10;" at head of the script.
...
Char::Latin10 software is "middleware" between perl interpreter and your Perl script
written in Latin-10.
...
By "use Char::Latin10;", it automatically interpret your script as Latin-10. The various
...
expand wildcard arguments supplied to programs. Instead, import of Char/Elatin10.pm
works well.
...
=head1 Calling 'Char::Elatin10::ignorecase()' (Char/Latin10.pm provides)
...
=head1 Escaping Character Classes (Char/Elatin10.pm provides)
...
=head1 Un-Escaping \ of \N, \p, \P and \X (Char/Latin10.pm provides)

Char/Latin10.pm removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (Char/Latin10.pm and Char/Elatin10.pm provide)

Insert 'Char::Elatin10::' at head of function name. Char/Elatin10.pm provides your script Char::Elatin10::*
functions.
...
=head1 Un-Escaping bytes::* Functions (Char/Latin10.pm provides)

Char/Latin10.pm removes 'bytes::' at head of function name.
...
=head1 Escaping Built-in Standard Module (Char/Elatin10.pm provides)

Char/Elatin10.pm does "BEGIN { unshift @INC, '/Perl/site/lib/Char::Latin10' }" at head.
Store the standard module modified for Char::Latin10 software in this directory to
override built-in standard modules.

=head1 Escaping Standard Module Content (You do)

You need copy built-in standard module to /Perl/site/lib/Char::Latin10 and change
'use utf8;' to 'use Char::Latin10;' in its. You need help yourself for now.
...
Char::Latin10 software remains one language and one interpreter.
