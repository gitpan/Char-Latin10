...
BEGIN {
    ...
    sub Char::Elatin10::binmode(*;$);
    sub Char::Elatin10::open(*;$@);
    ...
    if ($] < 5.006) {
        ...
        if ($^O ne 'MacOS') {
            *CORE::GLOBAL::binmode =
            *CORE::GLOBAL::binmode = \&Char::Elatin10::binmode;
        }
        *CORE::GLOBAL::open    =
        *CORE::GLOBAL::open    = \&Char::Elatin10::open;
    }
}
...
#               $pattern .= "(?:$your_char)",   # UNIX style
            ...
            elsif ((my $uc = Char::Elatin10::uc($char)) ne $char) {
                $pattern .= $uc;
            }
            else {
        ...
        my $matchsub = sub { Char::Elatin10::uc($_[0]) =~ m{\A $pattern \z}xms };
...
=item Split string
  ...
  Scans a Latin-10 $string for delimiters that match pattern and splits the Latin-10
  ...
  pattern, so the delimiters may be of any size and need not be the same Latin-10
  $string on every match. If the pattern doesn't match at all, Char::Elatin10::split returns
  the original Latin-10 $string as a single substring. If it matches once, you get
  ...
  If Latin-10 $string is omitted, the function splits the $_ Latin-10 string.
...
=item Chop string
  ...
  Chops off the last character of a Latin-10 string contained in the variable (or
  Latin-10 strings in each element of a @list) and returns the character chopped.
  The Char::Elatin10::chop operator is used primarily to remove the newline from the end of
  an input record but is more efficient than s/\n$//. If no argument is given, the
  function chops the $_ variable.

=item Index string

  $pos = Char::Elatin10::index($string,$substr,$position);
  $pos = Char::Elatin10::index($string,$substr);

  Returns the position of the first occurrence of $substr in Latin-10 $string.
  The start, if specified, specifies the $position to start looking in the Latin-10
  $string. Positions are integer numbers based at 0. If the substring is not found,
  the Char::Elatin10::index function returns -1.

=item Reverse index string

  $pos = Char::Elatin10::rindex($string,$substr,$position);
  $pos = Char::Elatin10::rindex($string,$substr);

  Works just like Char::Elatin10::index except that it returns the position of the last
  occurence of $substr in Latin-10 $string (a reverse index). The function returns
  -1 if not found. $position, if specified, is the rightmost position that may be
  returned, i.e., how far in the Latin-10 string the function can search.
...
=item Lower case string
  ...
  Returns a lowercase version of Latin-10 string (or $_, if omitted). This is the
...
=item Lower case first character of string
  ...
  Returns a version of Latin-10 string (or $_, if omitted) with the first character
...
=item Upper case string
  ...
  Returns an uppercased version of Latin-10 string (or $_, if string is omitted).
...
=item Make character
  ...
  set. For example, Char::Elatin10::chr(65) is "A" in either ASCII or Latin-10, and
  Char::Elatin10::chr(0x82a0) is a Latin-10 HIRAGANA LETTER A. For the reverse of Char::Elatin10::chr,
  use Char::Latin10::ord.
...
=item binary mode (Perl5.6 emulation on perl5.005)
  ...
  Char::Elatin10::binmode(FILEHANDLE, $disciplines);
  Char::Elatin10::binmode(FILEHANDLE);
  Char::Elatin10::binmode($filehandle, $disciplines);
  Char::Elatin10::binmode($filehandle);

  * two arguments

  If you are using perl5.005 other than MacPerl, Char::Latin10 software emulate perl5.6's
...
=item open file (Perl5.6 emulation on perl5.005)
  ...
  $rc = Char::Elatin10::open(FILEHANDLE, $mode, $expr);
  $rc = Char::Elatin10::open(FILEHANDLE, $expr);
  $rc = Char::Elatin10::open(FILEHANDLE);
  $rc = Char::Elatin10::open(my $filehandle, $mode, $expr);
  $rc = Char::Elatin10::open(my $filehandle, $expr);
  $rc = Char::Elatin10::open(my $filehandle);

  * autovivification filehandle
  * three arguments

  If you are using perl5.005, Char::Latin10 software emulate perl5.6's open function.
  ...
  {
      my $fh;                          # (uninitialized)
      Char::Elatin10::open($fh, ">logfile")     # $fh is autovivified
  ...
  Char::Elatin10::open my $fh, ">logfile" or die ...
  ...
  Char::Elatin10::open(LOG, ">", "logfile") or die "Can't create logfile: $!";
  ...
  $path =~ s#^([ ])#./$1#;
  Char::Elatin10::open (FH, "< $path\0") or die "can't open $path: $!";
  ...
  Char::Elatin10::open(FH, "<", $path) or die "can't open $path: $!";
  ...
  Char::Elatin10::open(FH, "<:raw", $path) or die "can't open $path: $!";
  ...
  Char::Elatin10::open(FH, "<:crlf:encoding(Char::Latin10)", $path) or die "can't open $path: $!";
